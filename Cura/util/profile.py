...
setting('infill_angle',               45, int,   'hidden',   _('Fill')).setRange(0, 100).setLabel(_("Infill angle (degree)"), _("Infill angle."))
setting('nozzle_size',             '0.4', float, 'advanced', _('Machine')).setLabel(_("Nozzle size (mm)"), _("The nozzle size is very important, this is used to calculate the line width of the infill, and used to calculate the amount of outside wall lines and thickness for the wall thickness you entered in the print settings."))
#setting('nozzle_size',             '0.4', [_('0.2'), _('0.4'), _('0.6'), _('0.8')], 'advanced', _('Machine')).setLabel(_("Nozzle size (mm)"), _("The nozzle size is very important, this is used to calculate the line width of the infill, and used to calculate the amount of outside wall lines and thickness for the wall thickness you entered in the print settings."))
setting('nozzle_type',              'V4',  [_('V4'),_('V5')], 'advanced', _('Machine')).setLabel(_("Nozzle Type"), _("Nozzle Type"))
...
setting('print_temperature3',        210, int,   'basic',    _('Speed and Temperature')).setRange(0,340).setLabel(_("3th nozzle temperature (C)"), _("Temperature used for printing. Set at 0 to pre-heat yourself.\nFor PLA a value of 210C is usually used.\nFor ABS a value of 230C or higher is required."))
setting('print_temperature4',        210, int,   'basic',    _('Speed and Temperature')).setRange(0,340).setLabel(_("4th nozzle temperature (C)"), _("Temperature used for printing. Set at 0 to pre-heat yourself.\nFor PLA a value of 210C is usually used.\nFor ABS a value of 230C or higher is required."))
setting('print_temperature5',        210, int,   'basic',    _('Speed and Temperature')).setRange(0,340).setLabel(_("5th nozzle temperature (C)"), _("Temperature used for printing. Set at 0 to pre-heat yourself.\nFor PLA a value of 210C is usually used.\nFor ABS a value of 230C or higher is required."))
...
setting('support_dual_extrusion', 'Second extruder', [_('First extruder'), _('Second extruder')], 'basic', _('Support')).setLabel(_("Support dual extrusion"), _("Which extruder to use for support material, for break-away support you can use both extruders.\nBut if one of the materials is more expensive then the other you could select an extruder to use for support material. This causes more extruder switches.\nYou can also use the 2nd extruder for soluble support materials."))
setting('wipe_tower',               True, bool,  'basic',    _('Dual extrusion')).setLabel(_("Wipe&prime tower"), _("The wipe-tower is a tower printed on every layer when switching between nozzles.\nThe old nozzle is wiped off on the tower before the new nozzle is used to print the 2nd color."))
...
setting('is_Encrypt_Gcode',        False, bool,  'hidden',    'hidden').setLabel(_("is Encrypt Gcode"), _("is Encrypt Gcode"))
...
setting('retraction_dual_amount',   15.0, float, 'advanced', _('Retraction')).setRange(0).setLabel(_("Dual extrusion switch amount (mm)"), _("Amount of retraction when switching nozzle with dual-extrusion, set at 0 for no retraction at all. A value of 16.0mm seems to generate good results."))
...
setting('retraction_min_travel',     1.5, float, 'hidden', 'hidden').setRange(0).setLabel(_("Minimum travel (mm)"), _("Minimum amount of travel needed for a retraction to happen at all. To make sure you do not get a lot of retractions in a small area."))
setting('retraction_combing',      'All',  [_('Off'),_('All'),_('No Skin')], 'hidden', 'hidden').setLabel(_("Enable combing"), _("Combing is the act of avoiding holes in the print for the head to travel over. If combing is \'Off\' the printer head moves straight from the start point to the end point and it will always retract.  If \'All\', enable combing on all surfaces.  If \'No Skin\', enable combing on all except skin surfaces."))
setting('retraction_minimal_extrusion',0.02, float, 'hidden', 'hidden').setRange(0).setLabel(_("Minimal extrusion before retracting (mm)"), _("The minimal amount of extrusion that needs to be done before retracting again if a retraction needs to happen before this minimal is reached the retraction is ignored.\nThis avoids retracting a lot on the same piece of filament which flattens the filament and causes grinding issues."))
...
setting('overlap_dual',                0, float, 'advanced', _('Quality')).setLabel(_("Dual extrusion overlap (mm)"), _("Add a certain amount of overlapping extrusion on dual-extrusion prints. This bonds the different colors together."))
...
setting('skirt_line_count',            0, int,   'expert', _('Skirt')).setRange(0).setLabel(_("Line count"), _("The skirt is a line drawn around the object at the first layer. This helps to prime your extruder, and to see if the object fits on your platform.\nSetting this to 0 will disable the skirt. Multiple skirt lines can help priming your extruder better for small objects."))
...
setting('fan_full_height',             0, float, 'expert',   _('Cool')).setRange(0).setLabel(_("Fan full on at height (mm)"), _("The height at which the fan is turned on completely. For the layers below this the fan speed is scaled linearly with the fan off at layer 0."))
...
setting('fill_overlap', 0, int, 'expert', _('Infill')).setRange(0,100).setLabel(_("Infill overlap (%)"), _("Amount of overlap between the infill and the walls. There is a slight overlap with the walls and the infill so the walls connect firmly to the infill."))
...
setting('support_z_distance', 0.3, float, 'expert', _('Support')).setRange(0,10).setLabel(_("Distance Z (mm)"), _("Distance from the top/bottom of the support to the print. A small gap here makes it easier to remove the support but makes the print a bit uglier.\n0.15mm gives a good seperation of the support material."))
...
setting('spiralize', False, bool, 'hidden', 'hidden').setLabel(_("Spiralize the outer contour"), _("Spiralize is smoothing out the Z move of the outer edge. This will create a steady Z increase over the whole print. This feature turns a solid object into a single walled print with a solid bottom.\nThis feature used to be called Joris in older versions."))
setting('simple_mode', False, bool, 'hidden', 'hidden').setLabel(_("Only follow mesh surface"), _("Only follow the mesh surfaces of the 3D model, do not do anything else. No infill, no top/bottom, nothing."))
setting('support_surface_fill_rate', 45, int, 'expert', _('Support')).setRange(0,100).setLabel(_("Fill amount of support surface (%)"), _("Fill amount for support surface."))
setting('support_surface_thickness', 1.5, float, 'expert', _('Support')).setRange(0,10).setLabel(_("The thickness of support surface (mm)"), _("The thickness of support surface."))
setting('support_surface_xy_distance', 2.1, float, 'expert', _('Support')).setRange(0,10).setLabel(_("Distance X/Y of support surface (mm)"), _("Distance of the support material from the print, in the X/Y directions.\n1mm gives a nice distance from the print so the support does not stick to the print."))
setting('support_surface_z_distance', 0.15, float, 'expert', _('Support')).setRange(0,10).setLabel(_("Distance Z of support surface (mm)"), _("Distance from the top/bottom of the support surface to the print. A small gap here makes it easier to remove the support but makes the print a bit uglier.\n0.2mm gives a good seperation of the support material."))
setting('add_support_surface', True, bool, 'expert', _('Support')).setLabel(_("Add support surface"), _("Add support surface."))
...
setting('fix_horrible_union_all_type_a', True,  bool, 'hidden', 'hidden').setLabel(_("Combine everything (Type-A)"), _("This expert option adds all parts of the model together. The result is usually that internal cavities disappear. Depending on the model this can be intended or not. Enabling this option is at your own risk. Type-A is dependent on the model normals and tries to keep some internal holes intact. Type-B ignores all internal holes and only keeps the outside shape per layer."))
setting('fix_horrible_union_all_type_b', False, bool, 'hidden', 'hidden').setLabel(_("Combine everything (Type-B)"), _("This expert option adds all parts of the model together. The result is usually that internal cavities disappear. Depending on the model this can be intended or not. Enabling this option is at your own risk. Type-A is dependent on the model normals and tries to keep some internal holes intact. Type-B ignores all internal holes and only keeps the outside shape per layer."))
setting('fix_horrible_use_open_bits', False, bool, 'hidden', 'hidden').setLabel(_("Keep open faces"), _("This expert option keeps all the open bits of the model intact. Normally Cura tries to stitch up small holes and remove everything with big holes, but this option keeps bits that are not properly part of anything and just goes with whatever is left. This option is usually not what you want, but it might enable you to slice models otherwise failing to produce proper paths.\nAs with all \"Fix horrible\" options, results may vary and use at your own risk."))
setting('fix_horrible_extensive_stitching', False, bool, 'hidden', 'hidden').setLabel(_("Extensive stitching"), _("Extensive stitching tries to fix up open holes in the model by closing the hole with touching polygons. This algorthm is quite expensive and could introduce a lot of processing time.\nAs with all \"Fix horrible\" options, results may vary and use at your own risk."))
...
setting('extruder_offset_x0',  '0.0', float, 'machine', 'hidden').setLabel(_("Offset X"), _("The offset of your secondary extruder compared to the primary."))
setting('extruder_offset_y0',  '0.0', float, 'machine', 'hidden').setLabel(_("Offset Y"), _("The offset of your secondary extruder compared to the primary."))
...
settingsDictionary['print_temperature2'].addCondition(lambda : int(getMachineSetting('extruder_amount')) > 1 and getProfileSetting('TIOON_enable') != 'True')
settingsDictionary['print_temperature3'].addCondition(lambda : int(getMachineSetting('extruder_amount')) > 2 and getProfileSetting('TIOON_enable') != 'True')
settingsDictionary['print_temperature4'].addCondition(lambda : int(getMachineSetting('extruder_amount')) > 3 and getProfileSetting('TIOON_enable') != 'True')
settingsDictionary['print_temperature5'].addCondition(lambda : int(getMachineSetting('extruder_amount')) > 4 and getProfileSetting('TIOON_enable') != 'True')
settingsDictionary['filament_diameter2'].addCondition(lambda : int(getMachineSetting('extruder_amount')) > 1 and getProfileSetting('TIOON_enable') != 'True')
settingsDictionary['filament_diameter3'].addCondition(lambda : int(getMachineSetting('extruder_amount')) > 2 and getProfileSetting('TIOON_enable') != 'True')
settingsDictionary['filament_diameter4'].addCondition(lambda : int(getMachineSetting('extruder_amount')) > 3 and getProfileSetting('TIOON_enable') != 'True')
settingsDictionary['filament_diameter5'].addCondition(lambda : int(getMachineSetting('extruder_amount')) > 4 and getProfileSetting('TIOON_enable') != 'True')
settingsDictionary['support_dual_extrusion'].addCondition(lambda : int(getMachineSetting('extruder_amount')) > 1)
settingsDictionary['retraction_dual_amount'].addCondition(lambda : int(getMachineSetting('extruder_amount')) > 1 and getProfileSetting('TIOON_enable') != 'True')
...
def resetMachine():
	if getMachineSetting('machine_name') == 'M1':
		putMachineSetting('machine_type', 'MakerPi')
		putMachineSetting('extruder_amount', 1)
		putMachineSetting('machine_width', 105.1)
		putMachineSetting('machine_depth', 105.1)
		putMachineSetting('machine_height', 105.1)
		putMachineSetting('has_heated_bed', 'False')
		putMachineSetting('extruder_head_size_min_x', '0')
		putMachineSetting('extruder_head_size_min_y', '0')
		putMachineSetting('extruder_head_size_max_x', '0')
		putMachineSetting('extruder_head_size_max_y', '0')
		putMachineSetting('extruder_head_size_height', '0')
		#putProfileSetting('raft_base_linewidth', 2.0)
		putProfileSetting('platform_adhesion', 'Raft')
		putProfileSetting('travel_speed', 80)
		putProfileSetting('fill_overlap', 4)
	elif getMachineSetting('machine_name') == 'M14':
		putMachineSetting('machine_type', 'MakerPi')
		putMachineSetting('extruder_amount', 1)
		putMachineSetting('machine_width', 145.1)
		putMachineSetting('machine_depth', 145.1)
		putMachineSetting('machine_height', 146.1)
		putMachineSetting('has_heated_bed', 'True')
		putMachineSetting('extruder_head_size_min_x', '75')
		putMachineSetting('extruder_head_size_min_y', '10')
		putMachineSetting('extruder_head_size_max_x', '53')
		putMachineSetting('extruder_head_size_max_y', '35')
		putMachineSetting('extruder_head_size_height', '50')
		putProfileSetting('nozzle_type', 'V4')
		putProfileSetting('platform_adhesion', 'Raft')
		putProfileSetting('travel_speed', 80)
	elif getMachineSetting('machine_name') == 'M15':
		putMachineSetting('machine_type', 'MakerPi')
		putMachineSetting('extruder_amount', 1)
		putMachineSetting('machine_width', 145.1)
		putMachineSetting('machine_depth', 145.1)
		...
		putProfileSetting('platform_adhesion', 'Raft')
		putProfileSetting('travel_speed', 80)
	elif getMachineSetting('machine_name') == 'M2030':
		putMachineSetting('machine_type', 'MakerPi')
		putMachineSetting('extruder_amount', 1)
		putMachineSetting('machine_width', 205.1)
		putMachineSetting('machine_depth', 205.1)
		putMachineSetting('machine_height', 306.1)
		putMachineSetting('extruder_head_size_min_x', '77')
		putMachineSetting('extruder_head_size_min_y', '13')
		putMachineSetting('extruder_head_size_max_x', '53')
		putMachineSetting('extruder_head_size_max_y', '35')
		putMachineSetting('extruder_head_size_height', '50')
		putProfileSetting('nozzle_type', 'V4')
	elif getMachineSetting('machine_name') == 'M2048':
		putMachineSetting('machine_type', 'MakerPi')
		putMachineSetting('extruder_amount', 1)
		putMachineSetting('machine_width', 205.1)
		putMachineSetting('machine_depth', 205.1)
		putMachineSetting('machine_height', 486.1)
		putMachineSetting('extruder_head_size_min_x', '77')
		putMachineSetting('extruder_head_size_min_y', '13')
		putMachineSetting('extruder_head_size_max_x', '53')
		putMachineSetting('extruder_head_size_max_y', '35')
		putMachineSetting('extruder_head_size_height', '50')
		putProfileSetting('nozzle_type', 'V4')
	elif getMachineSetting('machine_name') == 'K5/K6':
		putMachineSetting('machine_type', 'MakerPi')
		putMachineSetting('extruder_amount', 1)
		putMachineSetting('machine_width', 205.1)
		putMachineSetting('machine_depth', 205.1)
		putMachineSetting('machine_height', 306.1)
		putMachineSetting('extruder_head_size_min_x', '30.10')
		putMachineSetting('extruder_head_size_min_y', '27.80')
		putMachineSetting('extruder_head_size_max_x', '36.90')
		putMachineSetting('extruder_head_size_max_y', '62.60')
		putMachineSetting('extruder_head_size_height', '42.80')
		putProfileSetting('nozzle_type', 'V5')
		putProfileSetting('fill_overlap', 2)
		putProfileSetting('retraction_speed', 20.0)
		putProfileSetting('retraction_amount', 1.0)
		putProfileSetting('fill_overlap', 4)
	elif getMachineSetting('machine_name') == 'K300/M3145K/M3145':
		putMachineSetting('machine_type', 'MakerPi')
		putMachineSetting('extruder_amount', 1)
		putMachineSetting('machine_width', 315.1)
		putMachineSetting('machine_depth', 315.1)
		putMachineSetting('machine_height', 456.1)
		putMachineSetting('extruder_head_size_min_x', '15')
		putMachineSetting('extruder_head_size_min_y', '15')
		putMachineSetting('extruder_head_size_max_x', '48')
		putMachineSetting('extruder_head_size_max_y', '36')
		putMachineSetting('extruder_head_size_height', '57')
		putProfileSetting('nozzle_type', 'V5')
	elif getMachineSetting('machine_name') == 'K400/M4141/M41S':
		putMachineSetting('machine_type', 'MakerPi')
		putMachineSetting('extruder_amount', 1)
		putMachineSetting('machine_width', 415.1)
		putMachineSetting('machine_depth', 415.1)
		putMachineSetting('machine_height', 416.1)
		putMachineSetting('extruder_head_size_min_x', '35')
		putMachineSetting('extruder_head_size_min_y', '13')
		putMachineSetting('extruder_head_size_max_x', '13')
		putMachineSetting('extruder_head_size_max_y', '86')
		putMachineSetting('extruder_head_size_height', '50')
		putProfileSetting('nozzle_type', 'V4')
	elif getMachineSetting('machine_name') == 'S300/M3145S':
		putMachineSetting('machine_type', 'MakerPi')
		putMachineSetting('extruder_amount', 1)
		putMachineSetting('machine_width', 315.1)
		putMachineSetting('machine_depth', 315.1)
		putMachineSetting('machine_height', 456.1)
		putMachineSetting('extruder_head_size_min_x', '22')
		putMachineSetting('extruder_head_size_min_y', '18')
		putMachineSetting('extruder_head_size_max_x', '30')
		putMachineSetting('extruder_head_size_max_y', '76')
		putMachineSetting('extruder_head_size_height', '58')
		putProfileSetting('nozzle_type', 'V5')
	elif getMachineSetting('machine_name') == 'S400/S1':
		putMachineSetting('machine_type', 'MakerPi')
		putMachineSetting('extruder_amount', 1)
		putMachineSetting('machine_width', 415.1)
		putMachineSetting('machine_depth', 415.1)
		...
		putMachineSetting('extruder_head_size_min_x', '35')
		putMachineSetting('extruder_head_size_min_y', '13')
		profile.putMachineSetting('extruder_head_size_max_x', '13')
		putMachineSetting('extruder_head_size_max_y', '86')
		putMachineSetting('extruder_head_size_height', '50')
		putProfileSetting('nozzle_type', 'V4')
	elif getMachineSetting('machine_name') == 'X5/K5 MIX':
		putMachineSetting('machine_type', 'MakerPi')
		putMachineSetting('machine_width', 190.1)
		putMachineSetting('machine_depth', 205.1)
		putMachineSetting('machine_height', 306.1)
		putMachineSetting('TIOON_type', _('Gradient'))
		putProfileSetting('TIOON_enable', True)
		putProfileSetting('nozzle_type', 'V4')
		putProfileSetting('retraction_speed', 20.0)
		putProfileSetting('travel_speed', 100)
		putProfileSetting('fill_overlap', 4)
		putProfileSetting('is_Encrypt_Gcode', True)
		putProfileSetting('wipe_tower_volume', 100)
	elif getMachineSetting('machine_name') == 'X5 Dual':
		putMachineSetting('machine_type', 'MakerPi')
		putMachineSetting('machine_width', 190.1)
		putMachineSetting('machine_depth', 205.1)
		putMachineSetting('machine_height', 306.1)
		putMachineSetting('extruder_amount', 2)
		putMachineSetting('extruder_head_size_min_x', '75')
		putMachineSetting('extruder_head_size_min_y', '18')
		putMachineSetting('extruder_head_size_max_x', '18')
		putMachineSetting('extruder_head_size_max_y', '35')
		putMachineSetting('extruder_head_size_height', '50')
		putProfileSetting('nozzle_type', 'V4')
		putProfileSetting('travel_speed', 80)
		putProfileSetting('overlap_dual', 0.15)
		putProfileSetting('fill_overlap', 15)
		putProfileSetting('retraction_dual_amount', 20.0)
		putProfileSetting('retraction_speed', 20.0)
		putProfileSetting('retraction_amount', 4.5)
		putProfileSetting('solid_layer_thickness', 0.6)
		putProfileSetting('print_temperature', 180)
		putProfileSetting('print_temperature2', 180)
		putProfileSetting('wipe_tower', 'True')
		putProfileSetting('wipe_tower_volume', 50)
		putMachineSetting('extruder_offset_x1', 12)
		putProfileSetting('platform_adhesion', 'Raft')
		putProfileSetting('retraction_hop', 0.2)
		putProfileSetting('is_Encrypt_Gcode', True)
	elif getMachineSetting('machine_name') == 'M2030X':
		putMachineSetting('machine_type', 'MakerPi')
		putMachineSetting('machine_width', 190.1)
		putMachineSetting('machine_depth', 205.1)
		putMachineSetting('machine_height', 306.1)
		putMachineSetting('TIOON_type', _('Gradient'))
		putMachineSetting('extruder_offset_x0', -19.0)
		putProfileSetting('TIOON_enable', True)
		putProfileSetting('nozzle_type', 'V4')
		putProfileSetting('retraction_speed', 20.0)
		putProfileSetting('travel_speed', 100)
		putProfileSetting('is_Encrypt_Gcode', True)
		putProfileSetting('wipe_tower_volume', 100)
	elif getMachineSetting('machine_name') == 'M2048X':
		putMachineSetting('machine_type', 'MakerPi')
		putMachineSetting('machine_width', 190.1)
		putMachineSetting('machine_depth', 205.1)
		putMachineSetting('machine_height', 486.1)
...
def resetProfile():
	...
	if getMachineSetting('machine_type') == 'ultimaker' or getMachineSetting('machine_type') == 'MakerPi':
